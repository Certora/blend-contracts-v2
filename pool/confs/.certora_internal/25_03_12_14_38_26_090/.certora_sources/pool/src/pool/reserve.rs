...
impl Reserve {
    ...
    // certora munged
    pub fn compute_new_backstop_credit(&mut self, e: &Env, mut new_backstop_credit: i128, accrued: i128, bstop_rate: u32) -> i128 {
        let ret: i128 = cvlr::nondet();
        if bstop_rate > 0 {
            // floor(accrued * bstop_rate / SCALAR_7)
            let fz = ret * SCALAR_7;
            let xy = accrued * &i128(bstop_rate);

            cvlr_assume!(xy >= fz);
            cvlr_assume!(fz + SCALAR_7 > xy);
            new_backstop_credit = ret;
            self.data.backstop_credit += new_backstop_credit;
        }
        return new_backstop_credit
    }
    ...
    pub fn gulp(&mut self, e: &Env, bstop_rate: u32, accrued: i128) {
        ...
        if accrued > 0 {
            ...
            new_backstop_credit = self.compute_new_backstop_credit(e, new_backstop_credit, accrued, bstop_rate);
            // if bstop_rate > 0 {
