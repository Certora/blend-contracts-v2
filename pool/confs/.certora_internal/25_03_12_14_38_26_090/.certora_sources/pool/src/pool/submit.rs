...
pub fn execute_submit(
    ...
    if actions.check_health
        ...
        && positions_hf_under(e, &mut pool, &from_state.positions, 1_0000100)
    {
...
fn positions_hf_under(e: &Env, pool: &mut Pool, positions: &Positions, hf: i128) -> bool {
    PositionData::calculate_from_positions(e, pool, positions).is_hf_under(e, hf)
}
);
...
pub fn execute_submit_with_flash_loan(
    ...
    let actions =
        // We don't need to model this call here to prove the user health property
        // N.B. this also has side effects on some hidden fields of pool, but
        // we don't model it here since it's irrelevant to the properties we're checking
        nondet_expr!(
            from_state.positions;
